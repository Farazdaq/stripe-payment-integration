...
function clampRadius(r: number | undefined, fallback = 0) {
...
function buildPath(
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    // 👇 if radius = 0 → no rounding at all
    if (r.rb === 0 && r.lt === 0) {
      d += ` L ${curr.x} ${curr.y}`;
      continue;
    }
...
export default function LineShaper({
  ...
  // ✅ default is NOW 0 everywhere
  const r = {
    lt: clampRadius(radiusLeftTop, 0),
    lb: clampRadius(radiusLeftBottom, 0),
    rt: clampRadius(radiusRightTop, 0),
    rb: clampRadius(radiusRightBottom, 0),
  };
